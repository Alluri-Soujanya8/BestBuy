...
//	private static By verifyGiftIdeas = By.xpath("//h2[text()= 'Who are you shopping for?']");
	...
	public static boolean clickcheckbox()
	{
		...
		try
		{
			...
			driver.findElement(filterforsingle).click();
		}
		catch (TimeoutException te)
		{
			actResult = false;
	...
	public static boolean clickMultipleCheckbox() {
	    boolean actResult = true;
	    try {
	        // Wait for first checkbox
	        wait.until(ExpectedConditions.visibilityOfElementLocated(filterforsingle));
	        Reports.generateReport(driver, test, Status.PASS, "First checkbox is found");
	        driver.findElement(filterforsingle).click();
	...
	public static boolean clickClearAll()
	{
		...
		try
		{
			wait.until(ExpectedConditions.visibilityOfElementLocated(clearAll));
