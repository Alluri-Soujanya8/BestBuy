...
public class Profile {
	...
	//	public void click_on_clearall_button() {
	//		boolean status=userpage.clickClearAll();
	//		Assert.assertTrue(status);
	//
	//
	//	}
	...
	@And("click on continue as guest button")
	public void click_on_continue_as_guest() {
		String guestbutton=prop.getProperty("guest");
		userpage.clickContinueAsGuestButton();
	}
	@And("enter {int}{int} to place an order")
	public void enter_details_to_place_an_order(Integer sheet,Integer row) throws Exception {
		String path=prop.getProperty("path");
		ExcelReader excel=new ExcelReader(path);
		//String selectedLocality = ExcelReader.getColumnData(path, sheetNo, rowIndex);
		List<String> var = excel.getRowColData(sheet, row);

		String email = var.get(0);
		String phone = var.get(1);

		System.out.println("Email: " + email + ", Phone: " + phone);

		Assert.assertNotNull(var, "Locality not found at sheet " + sheet + ", row " + row);
		
	}
	@Then("click on payment button to place order")
	public void click_on_payment_button() {
		String payment=prop.getProperty("payment");
		userpage.clickPaymentButton();
