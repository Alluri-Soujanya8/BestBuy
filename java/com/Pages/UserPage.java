...
//    private static WebElement featuredDeals;
    ...
    private static WebElement Addtocart1;
    @FindBy (xpath = "(//button[text()='Add to cart'])[6]")
    private static WebElement Addtocart2;
...
//            toggleSwitch.click();
	...
	public boolean AddToCartFirstLaptop(int sheetNo, int rowNum) throws IOException
	{
        ...
        try 
        {
        	String[] firstAddToCart = ExcelReader.getRowData(sheetNo, rowNum); // Assuming row 1 = Add To Cart1
            wait.until(ExpectedConditions.visibilityOf(Addtocart1));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", Addtocart1);
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", Addtocart1);
	...
	public boolean AddToCartSecondLaptop(int sheetNo, int rowNum) throws IOException
	{
        ...
        try 
        {
        	String[] secondAddToCart = ExcelReader.getRowData(sheetNo, rowNum); // Assuming row 2 = Add To Cart 2
        	wait.until(ExpectedConditions.visibilityOf(Addtocart2));
        	((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", Addtocart2);
        	((JavascriptExecutor) driver).executeScript("arguments[0].click();", Addtocart2);
        	Reports.generateReport(driver, test, Status.PASS, "Add To Cart for second laptop is clicked");
        } 
        catch (TimeoutException | ElementClickInterceptedException e) 
        {
            actResult = false;
            Reports.generateReport(driver, test, Status.FAIL, "Add To Cart for second laptop is not clicked");
        }
        return actResult;
	}
